...
| `src/routes/login_route.py`        | Definición de los contratos de la API (Endpoints). Maneja el parsing de JSON y códigos de retorno HTTP. Genera el **Identity JWT** para saltos entre aplicaciones. |
| `src/utils/handshake.py`           | Generador de **Identity JWT (Fat Tokens)**. Empaqueta el contexto completo del usuario en un token firmado de corta duración.                                      |
...
| `SESSION_COOKIE_DOMAIN`            | String   | Dominio base para SSO (ej: `.prismgrp.com`). Permite que la cookie sea compartida entre subdominios. |
...
**Propósito:** Validación de identidad para APIs secundarias (Identity Handshake).

- **Request:** `{ "code": "JWT_TOKEN" }`
- **Seguridad:** Requiere `X-API-KEY`.
- **Lógica:**
  1. El "código" es ahora un **Identity JWT (Fat Token)**.
  2. Verifica la firma digital y la expiración (5 min).
  3. No requiere consulta a SeaTable (es instantáneo).
- **Response:** `{ "success": true, "valid": true, "email": "...", "user": { CONTEXTO_COMPLETO } }`

---

## 11. Guía de Validación Local (SSO para Desarrolladores)

Para evitar latencia de red, las aplicaciones secundarias (CRM, Insights) pueden validar el `handshake_code` localmente.

### 11.1. Paso a paso para APIs Secundarias

1.  **Recibir el Token:** El frontend envía el `handshake_code` en el header `Authorization: Bearer <TOKEN>`.
2.  **Validación de Firma:** Usar una librería de JWT (ej: `PyJWT` en Python, `jsonwebtoken` en Node).
3.  **Uso de Datos:** El JSON decodificado contiene una propiedad `user` con todo el perfil, roles y permisos.

```python
# Ejemplo de validación local en Python
import jwt
payload = jwt.decode(token, "SHARED_SECRET_KEY", algorithms=["HS256"])
user_context = payload['user']
```
